...
describe("RatingGroup", () => {
  ...
  test("renders one item per star in the range", async () => {
    ...
    expect(
      screen.container.querySelectorAll("[data-rating-group-item]"),
    ).toHaveLength(5);
  ...
  test("invokes onValueChange when a star is selected", async () => {
    ...
    const items = screen.container.querySelectorAll("[data-rating-group-item]");
    await page.elementLocator(items[2]).click();
